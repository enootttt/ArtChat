...
- Notification为系统应用通知，受操作系统通知权限管控，如果系统通知权限被关闭，ArtNotification的 open 方法调用将无任何效果。系统权限设置。
- ArtNotification 是由扩展 window.Notification实现的，如果浏览器环境不支持Notification，ArtNotification的方法调用将无任何效果。
- ArtNotification 通知样式与效果均已当前浏览器环境对Notification的支持为准，例如dir属性会被大部分浏览器忽略。
- ArtNotification 仅对当前实例下的通知进行关闭管理，实例变更后（例：浏览器页面刷新）对已发送的通知无管理关闭能力。

## 代码演示

### 基本

:::demo hooks调用。发送通知前需要向用户请求通知权限，授权可通知后可发送通知， 若授权禁止通知则不可以发送通知。
...
notification/close_tag

:::

### 静态方法

:::demo 静态方法调用。发送通知前需要向用户请求通知权限，授权可通知后可发送通知， 若授权禁止通知则不可以发送通知。
...
| open              | 向用户推送一个通知                                                      | (config: ArtNotificationOpenArgs)=> void | -      | -    |
...
#### ArtNotificationOpenArgs

```tsx | pure
interface ArtNotificationOpenArgs {
...
type useNotification = [
  { permission: ArtNotification['permission'] },
  {
    open: ArtNotification['open']
    close: ArtNotification['close']
    requestPermission: ArtNotification['requestPermission']
  },
]
```
