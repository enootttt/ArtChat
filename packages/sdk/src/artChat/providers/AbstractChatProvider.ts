...
import type {
  AbstractArtRequestClass,
  ArtRequestCallbacks,
  ArtRequestOptions,
} from '../../artRequest'

export interface ChatProviderConfig<Input, Output> extends AnyObject {
  request: AbstractArtRequestClass<Input, Output> | (() => AbstractArtRequestClass<Input, Output>)
}
...
export default abstract class AbstractChatProvider<ChatMessage, Input, Output> {
  private _request!: AbstractArtRequestClass<Input, Output>
  private _getMessagesFn!: () => ChatMessage[]
  private _originalCallbacks?: ArtRequestCallbacks<Output>
