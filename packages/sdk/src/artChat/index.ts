...
import type { ConversationData } from '../artConversations'
import type { AbstractArtRequestClass } from '../artRequest'
import type { SSEOutput } from '../artStream'
...
export default function useArtChat<
  ...
  Output = SSEOutput,
>(config: ArtChatConfig<ChatMessage, ParsedMessage, Input, Output>) {
  const {
    defaultMessages,
    requestFallback,
    requestPlaceholder,
  ...
  const parsedMessages = computed(() => {
    ...
    messages.value.forEach((agentMsg) => {
      const rawParsedMsg = parser ? parser(agentMsg.message as ChatMessage) : agentMsg.message
      ...
      if (!msg) {
        if (reload && updatingId) {
          ...
          if (msg) {
            ...
            setMessages((ori: MessageInfo<ChatMessage>[]) => [...ori])
  ...
  const onRequest = (requestParams: Partial<Input>) => {
    if (!provider) {
