...
function doRenderIframe() {
  ...
  const hasUtf8Meta = /<head[^>]*>[\s\S]*?<meta\s[^>]*charset=["']?utf-8["']?/i.test(sanitizedHtml)

  if (hasHead) {
    if (!hasUtf8Meta) {
      finalHtml = sanitizedHtml.replace(/<head[^>]*>/i, (match) => `${match}<meta charset="UTF-8">`)
    }
  } else {
...
<template>
  <ElScrollbar ref="codeContainerRef" class="elx-run-code-content-scrollbar" :style="preStyle">
    <div
