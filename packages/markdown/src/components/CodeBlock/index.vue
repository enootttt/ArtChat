...
const { codeXSlot, customAttrs, enableCodeLineNumber = false, globalShiki } = toValue(context) || {}
...
const viewCodeModalOptions = computed(() => context?.value?.viewCodeModalOptions)
const isExpand = ref(true)
const nowCodeLanguage = ref<BundledLanguage>()
const codeAttrs =
  typeof customAttrs?.code === 'function' ? customAttrs.code(props.raw) : customAttrs?.code || {}
...
async function generateHtml() {
  ...
  const html = await codeToHtml(content.trim(), {
    ...
    transformers: shikiTransformers as ShikiTransformer[],
  })
...
const RunCodeComputed = computed(() => {
  return nowCodeLanguage.value === 'html' && nowViewBtnShow.value ? RunCode : undefined
})
