...
import type { ArtRequestParams } from '../artRequest'
import type { ArtStreamOptions, SSEOutput } from '../artStream'
import type { ArtAgent } from '../useArtAgent'
...
export default function useArtChat<
  ...
  Output = SSEOutput,
>(config: ArtChatConfig<AgentMessage, ParsedMessage, Input, Output>) {
  const {
    defaultMessages,
    agent,
    requestFallback,
  ...
  const getTransformMessage: TransformMessageFn<AgentMessage, Output> = (params) => {
    ...
    if (Array.isArray(chunks)) {
      ...
      if (originMessage) {
        // If originMessage is provided, return it
        return originMessage
      }
      // If originMessage is not provided, return the last chunk
      return chunk as unknown as AgentMessage
    }
